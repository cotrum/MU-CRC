...
function createKeypad() {
   const keypadItems = [];
   for (let i = 0; i < 10; i++) {
       const numberPadItem = React.createElement('div', {
           className: 'grid-item',
           key: i,
           ref: (element) => {
               if (element) {
                   // Attach the click handler to the DOM element
                   element.addEventListener('click', () => checkButtonPress(element));
               }
           },
       }, (i + 1) % 10);
       keypadItems.push(numberPadItem);
   }
   return React.createElement('div', { className: 'keypad-container' }, keypadItems);
}
...
function addKeypadToGameFrame() {
   ...
   if (gameFrame) {
       ...
       const root = ReactDOM.createRoot(gameFrame);

       // Create the level count display
       const levelCount = React.createElement('div', { className: 'level-count' }, `Level count: ${currentLevel}/${totalLevels}`);

       // Create the keypad container
       const keypadContainer = createKeypad();

       // Render the React elements into the root
       root.render(
           React.createElement(React.Fragment, null, levelCount, keypadContainer)
       );
   }
}
...
async function animateKeypad() {
    ...
    if (!started) {
        ...
        for (let i = 0; i < totalLevels; i++) {
            ...
            for (let j = 0; j <= i; j++) {
                const index = (randomNumberSequence[j] - 1 < 0) ? 9 : randomNumberSequence[j] - 1;
...
function checkWin(timeout) {
    ...
    if (!timeout) {
        ...
        // Get the elapsed time and add it to the cumulative time
